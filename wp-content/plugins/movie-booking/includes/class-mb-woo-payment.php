...
if ( ! class_exists( 'MB_Woo_Payment' ) ) {
    ...
    class MB_Woo_Payment {
        ...
        public function __construct() {
            ...
            add_filter( 'woocommerce_order_item_name' , array( $this, 'mb_display_cart_movie_name' ), 10, 3 );
        ...
        public function mb_display_cart_movie_permalink( $link, $cart_item, $cart_item_key ) {
            if ( isset( $cart_item['booking_id'] ) && $cart_item['booking_id'] ) {
                $prefix     = MB_PLUGIN_PREFIX_BOOKING;
                $movie_id   = get_post_meta( $cart_item['booking_id'], $prefix.'movie_id', true );
        ...
        public function mb_display_cart_calculate_totals( $wc_cart ) {
            ...
            foreach( $wc_cart->get_cart() as $cart_item_key => $cart_item ) {
                if ( isset( $cart_item['booking_id'] ) && $cart_item['booking_id'] ) {
                    ...
                    // Discount
                    $total_discount = MB_Cart()->get_price_discount( $discount_code, $movie_id, $subtotal );
                    ...
                    // Tax
                    $total_tax = MB_Cart()->get_price_tax( $subtotal );

                    // Ticket Fee
                    $total_ticket_fee = MB_Cart()->get_price_ticket_fee( $subtotal );

                    // Total
                    $number_decimals = MBC()->mb_get_currency_minor_unit();

                    $total = round( floatval( $subtotal + $total_tax + $total_ticket_fee*$count_seat ), $number_decimals );
                    ...
                    if ( $total_discount ) {
                        ...
                        <tr class="mb-cart-discount">
                            ...
                            <td data-title="<?php esc_attr_e( 'Discount', 'moviebooking' ); ?>">
                                <?php echo esc_html__( '-', 'moviebooking' ) . wc_price( $total_discount ) . sprintf( esc_html__( ' (%s)', 'moviebooking' ), $discount_code ); ?>
                            </td>
                        </tr>
                        <?php
                    }
                    ...
                    if ( $total_ticket_fee ) {
                        ...
                        <tr class="mb-ticket-fee">
                            ...
                            <td data-title="<?php esc_attr_e( 'Ticket Fee', 'moviebooking' ); ?>">
                                <?php echo wc_price( $total_ticket_fee * $count_seat ); ?>
                            </td>
                        </tr>
                        <?php
                    }
        ...
        public function mb_display_cart_totals_order_total_html( $total ) {
            if ( is_admin() && ! defined( 'DOING_AJAX' ) ) return;
            ...
            foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
                if ( isset( $cart_item['booking_id'] ) && $cart_item['booking_id'] ) {
                    ...
                    // Total
                    $total = '<strong>' . wc_price( $subtotal + $total_tax + $total_ticket_fee*$count_seat ) . '</strong>';

                    // Update Cart session
                    $number_decimals = MBC()->mb_get_currency_minor_unit();

                    WC()->cart->cart_contents[$cart_item_key]['subtotal']   = round( $subtotal, $number_decimals );
                    WC()->cart->cart_contents[$cart_item_key]['discount']   = round( $total_discount, $number_decimals );
                    WC()->cart->cart_contents[$cart_item_key]['tax']        = round( $total_tax, $number_decimals );
                    WC()->cart->cart_contents[$cart_item_key]['ticket_fee'] = round( $total_ticket_fee, $number_decimals );
                    WC()->cart->cart_contents[$cart_item_key]['total']      = round( floatval( $subtotal + $total_tax + $total_ticket_fee*$count_seat ), $number_decimals );
                    WC()->cart->set_session();
        ...
        public function mb_checkout_create_order_line_item( $item, $cart_item_key, $values, $order ) {
            if ( isset( $values['booking_id'] ) && $values['booking_id'] ) {
                ...
                if ( floatval( $values['discount'] ) ) {
                    $values['subtotal'] = floatval( $values['subtotal'] ) + $values['discount'];
                }
                ...
                foreach( $booking_update as $k => $v ) {
                    // Update Booking
                    update_post_meta( $booking_id, $prefix.$k, $v );
                    ...
                    if ( $k === 'cart_subtotal' ) {
                        ...
                        if ( $ticket_fee ) {
                            $total_rows_new['mb_ticket_fee'] = array(
                                ...
                                'value' => wc_price( $ticket_fee*$qty , array( 'currency' => $order->get_currency() ) ),
                            );
                        }
                    }
                }
                ...
                if ( $ticket_fee ) {
                    ...
                    <tr>
                        ...
                        <td class="mb_ticket_fee">
                            <div class="view">
                                <?php echo wc_price( $ticket_fee*$qty , array( 'currency' => $order->get_currency() ) ); ?>
                            </div>
                        </td>
                    </tr>
                    <?php
